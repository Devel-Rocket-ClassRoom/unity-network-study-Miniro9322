    ...
    public class CoinArenaManager : NetworkBehaviour
    {
        ...
        private void TrySpawnCoin()
        {
            if (m_State.Value == GameState.Playing)
            {
                ...
                Vector3 position = new Vector3(UnityEngine.Random.Range(-m_SpawnAreaRadius, m_SpawnAreaRadius), 0.5f, UnityEngine.Random.Range(-m_SpawnAreaRadius, m_SpawnAreaRadius));

                var instance = Instantiate(m_CoinPrefab, position, Quaternion.identity);
