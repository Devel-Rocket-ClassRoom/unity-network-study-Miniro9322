...
public class RelayBootstrap : MonoBehaviour
{
    ...
    public async UniTask EnsureSingedInAsync()
    {
    ...
    public async UniTask<string> StartHostWithRelayAsync(int maxConnection)
    {
        ...
        var serverData = AllocationUtils.ToRelayServerData(allocation, m_ConnectionType);
        GetUnityTransport().SetRelayServerData(serverData);

        var started = m_NetworkManager.StartHost();
        if (!started)
        {
    ...
    public async UniTask StartClientWithRelayAsync(string joinCode)
    {
        ...
        var serverData = AllocationUtils.ToRelayServerData(joinAllocation, m_ConnectionType);
        GetUnityTransport().SetRelayServerData(serverData);

        var started = m_NetworkManager.StartClient();
        if (!started)
        {
            throw new Exception();
        }
